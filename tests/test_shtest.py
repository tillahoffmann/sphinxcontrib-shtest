...
def test_strip_colors() -> None:
    colored = (
        f"{colorama.Fore.GREEN}hello {colorama.Fore.RED}world{colorama.Fore.RESET}"
    )
...
def test_directive_build(
    tmp_path: Path, document_name: str, match: Optional[str], builder: str
) -> None:
    ...
    (tmp_path / "index.rst").write_text(
        "\n".join([document_name, "=" * len(document_name), "", text])
    )
